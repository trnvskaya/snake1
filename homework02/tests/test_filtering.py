...
from filtering.helpers import read_image, identity_kernel, approx_gaussian_blur_5_kernel, edge_detection_kernel, \
    roberts_cross_1_kernel, roberts_cross_2_kernel, shift_kernel
...
def image_shifted():
    """ Loads image with roverts cross kernel applied """
    return read_image('tests/lenna_shifted.png')


@pytest.fixture(scope="session")
def linter():
    """ Test codestyle for src file. """
...
def test_shift(image, image_shifted):
    """ Test shift kernel """
    assert_equal(apply_filter(image, shift_kernel), image_shifted)
